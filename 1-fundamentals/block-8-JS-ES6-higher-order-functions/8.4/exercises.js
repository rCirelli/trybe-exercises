...
const bestSubject = (arr) => arr.reduce((accSubject, currSubject) => {
  // console.log(`acc: ${accSubject.nota} | curr: ${currSubject.nota}`)
  return (accSubject.nota > currSubject.nota) ? accSubject : currSubject
});
...
const numbers = [18, 19, 230, 53, 4, 5, 76, 23, 54];

// somar todos números pares do array

// const sumNumbers = (result, number) => result + number;

const isEven = (array) => ( array.filter((number) => {
  if (number % 2 === 0) return number;
}));

const sumEvenNumbers = (array) => isEven(array).reduce((result, number) => result + number);
